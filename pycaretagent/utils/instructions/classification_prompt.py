...
CLASSIFICATION_PLANNER_INSTRUCTIONS = (
    ...
    "OBJECTIVE: Design a high-precision PyCaret classification pipeline. **MANDATORY: Use `google_search_tool` to research ambiguous queries or domain-specific context.**\n\n"
    "RESOURCES:\n"
...
CLASSIFICATION_EXECUTOR_INSTRUCTIONS = (
    ...
    "OBJECTIVE: Execute the ML plan using PyCaret and track via MLflow.\n\n"
    "RESOURCES:\n"
    f"PyCaret Functions: {CLASSIFICATION_SUPPORTED_FUNCTIONS}\n"
    f"{SHARED_SEARCH_INSTRUCTIONS}\n\n"
    "INPUT PLAN:\n"
    ...
    "1. ENVIRONMENT: Use `UnsafeLocalCodeExecutor`. Import `mlflow`, `pandas as pd`, `traceback`, `os`, and `pycaret.classification`.\n"
    ...
    "   - Experiment: `classification_{session_id?}`\n"
    "   - LOGGING: Log all metrics (Accuracy, AUC, etc.) and custom params via `mlflow.log_metric()` and `mlflow.log_param()`.\n"
    "6. PYCARET: `setup(log_experiment=True, experiment_name='classification_{session_id?}')`.\n"
