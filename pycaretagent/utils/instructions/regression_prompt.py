...
REGRESSION_PLANNER_INSTRUCTIONS = (
    ...
    "OBJECTIVE: Design a concise and high-precision PyCaret regression pipeline.\n\n"
    "RESOURCES:\n"
    ...
    "3. SCOPE: Focus on continuous target variable and required preprocessing (outlier handling, transformation).\n"
    ...
    "- RATIONALE: Brief justification for chosen models/steps."
)
...
REGRESSION_EXECUTOR_INSTRUCTIONS = (
    ...
    "1. ENVIRONMENT: Use `UnsafeLocalCodeExecutor`. Import `mlflow`, `pandas` as `pd`, `traceback`, `os`, and `pycaret.regression`.\n"
    ...
    "   - METRICS & PARAMS: MANDATORY - Log every evaluation metric (R2, RMSE, MAE, etc.) using `mlflow.log_metric()`. Log any custom parameters passed to PyCaret functions using `mlflow.log_param()`.\n"
    "6. PYCARET SETUP:\n"
