...
ANOMALY_PLANNER_INSTRUCTIONS = (
    ...
    "OBJECTIVE: Design a concise and high-precision PyCaret anomaly detection pipeline.\n\n"
    "RESOURCES:\n"
    ...
    "- RATIONALE: Brief justification for chosen algorithms."
)
...
ANOMALY_EXECUTOR_INSTRUCTIONS = (
    ...
    "1. ENVIRONMENT: Use `UnsafeLocalCodeExecutor`. Import `mlflow`, `pandas` as `pd`, `traceback`, `os`, and `pycaret.anomaly`.\n"
    ...
    "   - METRICS & PARAMS: MANDATORY - Log every evaluation metric using `mlflow.log_metric()`. Log any custom parameters passed to PyCaret functions using `mlflow.log_param()`.\n"
    "6. PYCARET SETUP:\n"
    "   - Call `setup()` with `log_experiment=True` and `experiment_name='anomaly_{session_id?}'`.\n"
